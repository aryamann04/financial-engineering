...
class SVI: 
    def __init__(self, S_0, T, r, q, option_type, fetcher): 
        self.S_0 = S_0
        self.T = T
        self.r = r
        self.q = q 
        ...
        self.strikes, self.implied_vols, _ = fetcher.plot_implied_vols(r=r, option_type=option_type, plot=False)
    ...
    def plot_svi(self):         
        ...
        ax.plot(self.strikes, self.svi_vols, label='SVI-calibrated vols', marker='o', linestyle='--', color="orange")
        ax.axvline(x=self.S_0, color='black', linestyle='--', label=f'current price: ${self.S_0:.2f}')
        
        ax.set_xlabel('strikes')
        ax.set_ylabel(f'implied vols (%)')
        ax.set_title(f'{self.ticker} SVI Vol Surface (expiry {self.T:.2f} years) (calibrated with {self.option_type}s)')
        ax.legend()
        ax.grid(True)
